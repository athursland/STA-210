...
airbnb <- airbnb %>% mutate(inzip28801 = ifelse(zipcode == 28801, "yes", "no"))
...
ggplot(train.airbnb, aes(x=bed_type, y=price)) + geom_boxplot() + labs(title="Distribution of nightly price by bed type")

#Boxplot of price by room type
ggplot(train.airbnb, aes(x=room_type, y=price)) + geom_boxplot() + labs(title="Distribution of nightly price by room type")

ggplot(train.airbnb, aes(x=cleaning_fee)) + geom_histogram(stat="count") + labs(title="Distribution of cleaning fees")
...
>>>>>>> d09f48e490f8137378aeefee7fc2d832308fba8b
```
...
interactions.model <- lm(logprice ~ cleaning_fee * bathrooms + cleaning_fee * accommodates + room_type * bathrooms + accommodates * bathrooms + cleaning_fee * log.review_scores_cleanliness + zipcode * log.review_scores_location + availability_30 * minimum_nights + cancellation_policy * accommodates + host_is_superhost
+ host_listings_count
+ zipcode
+ room_type
+ accommodates
...
backwards.interactions.model <- ols_step_backward_aic(interactions.model, details=FALSE)
```

```{r final-interactions-model}
backwards.interactions.model <- lm(logprice ~ cleaning_fee * accommodates + availability_30 * minimum_nights + host_is_superhost
+ zipcode
+ room_type
+ accommodates
+ bathrooms
...
kable(tidy(backwards.interactions.model), format="markdown", digits = 4)
kable(glance(backwards.interactions.model))
```
...
#Self Model
#```{r full model with zip changes}
full.model.z <- lm(logprice ~ host_is_superhost
+ host_listings_count
+ zipcode
+ room_type
+ accommodates
...
+ cancellation_policy
+ inzip28801
+ inzip28804, data=train.airbnb)

kable(tidy(full.model.z), format="markdown", digits = 4)
kable(glance(full.model.z))
...
#backward.log.model.z <- ols_step_backward_aic(full.model.z)
#```

# Assumptions

```{r resids 2}
train.airbnb <- train.airbnb %>% mutate(stand.resid = rstandard(backwards.interactions.model),
                                        pred = predict(backwards.interactions.model))
...
p4 <- ggplot(data = train.airbnb, aes(x=accommodates, y=stand.resid)) + geom_point() + 
  labs(x="Number of Guests", y="Residual", title="Residuals vs Accommodates",
subtitle=("backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p5 <- ggplot(data = train.airbnb, aes(x=bathrooms, y=stand.resid)) + geom_point() + 
  labs(x="Number of Bathrooms", y="Residual", title="Residuals vs Bathrooms",
subtitle=("final.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p6 <- ggplot(data = train.airbnb, aes(x= cleaning_fee, y=stand.resid)) + geom_point() + 
  labs(x="Fee ($)", y="Residual", title="Residuals vs Cleaning Fee",
subtitle=("Backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p7 <-ggplot(data = train.airbnb, aes(x=minimum_nights, y=stand.resid)) + geom_point() + 
  labs(x="Minimum Nights", y="Residual", title="Residuals vs Minimum_nights",
subtitle=("Backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p8 <- ggplot(data = train.airbnb, aes(x=availability_30, y=stand.resid)) + geom_point() + 
  labs(x="Number of Available Nights in the next month", y="Residual", title="Residuals vs Availability",
subtitle=("backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p9 <- ggplot(data = train.airbnb, aes(x=log.review_scores_cleanliness, y=stand.resid)) + geom_point() + 
  labs(x="Cleanliness Review (out of 100%)", y="Residual", title="Residuals vs Log Cleanliness Review Scores",
subtitle=("backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))


p10 <- ggplot(data = train.airbnb, aes(x=log.review_scores_accuracy, y=stand.resid)) + geom_point() + 
  labs(x="Location Review (out of 100%)", y="Residual", title="Residuals vs Log Review Accuracy",
subtitle=("backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p11 <- ggplot(data = train.airbnb, aes(x=log.reviews_per_month, y=stand.resid)) + geom_point() + 
  labs(x="Reviews per month", y="Residual", title="Residuals vs Log Reviews per month",
subtitle=("backwards.interactions.model"))+
theme(plot.title = element_text(hjust = 0.5,size=14),
plot.subtitle=element_text(hjust=0.5,size=10))

p12 <- ggplot(data = train.airbnb, aes(x=cancellation_policy, y=stand.resid)) + geom_point() + 
...
train.airbnb <- train.airbnb %>%
  mutate(leverage = hatvalues(backwards.interactions.model), 
         cooks = cooks.distance(backwards.interactions.model),
         obs.num = row_number())
...
```
=======
test.airbnb <- test.airbnb %>% mutate(predict = predict.lm(backwards.interactions.model, test.airbnb))
>>>>>>> d09f48e490f8137378aeefee7fc2d832308fba8b

# residuals
test.resid2 <- test.airbnb$logprice - test.airbnb$predict
test.airbnb <- cbind(test.airbnb,test.resid2)
...
# Self Assumptions: Final Assumptions
...
mse.train <- summary(self.model)$sigma^2
mse.test  <- sum((test.self.resid)^2)/(nrow(test.airbnb)-2)
...
=======
test.airbnb<- test.airbnb[, !duplicated(colnames(test.airbnb))]
```

```{r residual plot for test data}
...
ggplot(test.airbnb, aes(x=predict, y=logprice)) + geom_point()
```
>>>>>>> d09f48e490f8137378aeefee7fc2d832308fba8b
